...
exports.handler = async (event) => {
  ...
  for (let i = 0; i < 5; i++) {
    const check = await sbRequest('GET', `access_codes?code=eq.${code}&select=id`, null);
    if (check.status === 200 && Array.isArray(check.data) && check.data.length === 0) break;
    code = generateCode();
  }

  // 2. access_codes tablosuna ekle
  ...
  if (codeRes.status !== 201) {
    return { statusCode: 500, body: JSON.stringify({ ok: false, error: 'Kod oluşturulamadı: ' + JSON.stringify(codeRes.data) }) };
  }
  ...
  return {
    statusCode: 200,
    body: JSON.stringify({ ok: true, code, email: email || null, telefon: telefon || null, firma: firma || '' })
  };
};
