...
async function probeSmart(url) {
  let r = await probeUrl(url, 'HEAD');
  if (r.reachable) return r;
  return probeUrl(url, 'GET');
}
...
exports.handler = async (event) => {
  ...
  const body = {
    ...
    summary,
    note:
      'reachable=true → TCP+TLS ile host:port açıldı; httpStatus (401/405/415 vb.) normal olabilir. reachable=false + ECONN/TLS/Zaman aşımı → port/firewall/yol sorunu. dns_ipv4 değerlerini bankanın verdiği 195.142… listesi ile karşılaştırın. Gerçek MPI için XML body gerekir.',
  };
