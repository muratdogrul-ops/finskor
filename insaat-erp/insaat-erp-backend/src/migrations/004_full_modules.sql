...
-- Depo / stok_kalem hareketleri (şantiye stok_hareketleri ile karışmasın diye ayrı isim)
CREATE TABLE IF NOT EXISTS depo_stok_hareketleri (
  id            UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id     UUID NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_depo_stok_har_depo ON depo_stok_hareketleri(depo_id, olusturuldu DESC);
